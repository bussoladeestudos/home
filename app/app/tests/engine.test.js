...
/* ── calcExercicios: base do Painel de Exercícios do dashboard ── */

function engineComExDias(exDias){
  global.QUESTOES=FIX_QUESTOES;
  return freshEngine(baseState({exDias:exDias||{}}),FIX_EDITAIS);
}

test("calcExercicios: sem nenhuma resposta devolve objeto vazio, e não null",()=>{
  const r=engineComExDias().calcExercicios("7","2026-01-20");
  assert.strictEqual(r.temDados,false);
  assert.strictEqual(r.total,0);
  assert.strictEqual(r.pct,0);
  assert.strictEqual(r.serie.length,8);        // a série existe, zerada
  assert.strictEqual(r.serie.every(d=>d.n===0),true);
});

test("calcExercicios: o período recorta o anel de desempenho",()=>{
  const E=engineComExDias({
    "2026-01-20":{n:10,ok:8},   // hoje
    "2026-01-18":{n:10,ok:5},   // dentro de 7 dias
    "2026-01-05":{n:10,ok:1},   // dentro de 30, fora de 7
    "2025-11-10":{n:10,ok:0}    // só em "sempre"
  });
  assert.strictEqual(E.calcExercicios("hoje","2026-01-20").total,10);
  assert.strictEqual(E.calcExercicios("hoje","2026-01-20").pct,80);
  assert.strictEqual(E.calcExercicios("7","2026-01-20").total,20);
  assert.strictEqual(E.calcExercicios("7","2026-01-20").pct,65);      // 13 de 20
  assert.strictEqual(E.calcExercicios("30","2026-01-20").total,30);
  assert.strictEqual(E.calcExercicios("sempre","2026-01-20").total,40);
  assert.strictEqual(E.calcExercicios("sempre","2026-01-20").acertos,14);
  assert.strictEqual(E.calcExercicios("sempre","2026-01-20").erros,26);
});

test("calcExercicios: a série de barras acompanha o período",()=>{
  const E=engineComExDias({"2026-01-20":{n:5,ok:3}});
  assert.strictEqual(E.calcExercicios("hoje","2026-01-20").serie.length,8);
  assert.strictEqual(E.calcExercicios("7","2026-01-20").serie.length,8);
  assert.strictEqual(E.calcExercicios("30","2026-01-20").serie.length,30);
  assert.strictEqual(E.calcExercicios("sempre","2026-01-20").serie.length,30);
  const s=E.calcExercicios("7","2026-01-20").serie;
  assert.strictEqual(s[s.length-1].key,"2026-01-20");   // hoje é a última barra
  assert.strictEqual(s[s.length-1].hoje,true);
  assert.strictEqual(s[s.length-1].n,5);
  assert.strictEqual(s[0].n,0);                          // dia sem resposta vale zero
});

test("calcExercicios: média diária e variação são sempre de 7 dias contra os 7 anteriores",()=>{
  const dias={};
  for(let i=0;i<7;i++){ const d=new Date(2026,0,20); d.setDate(d.getDate()-i);
    dias[d.toISOString().slice(0,10)]={n:10,ok:7}; }            // 70 nesta semana
  for(let i=7;i<14;i++){ const d=new Date(2026,0,20); d.setDate(d.getDate()-i);
    dias[d.toISOString().slice(0,10)]={n:5,ok:3}; }             // 35 na anterior
  const r=engineComExDias(dias).calcExercicios("7","2026-01-20");
  assert.strictEqual(r.mediaDiaria,10);
  assert.strictEqual(r.variacao,100);                            // dobrou
  // a medida não muda com o período escolhido: o rótulo diz "(7 dias)"
  assert.strictEqual(engineComExDias(dias).calcExercicios("sempre","2026-01-20").mediaDiaria,10);
});

test("calcExercicios: sem semana anterior, a variação é nula em vez de infinita",()=>{
  const r=engineComExDias({"2026-01-20":{n:10,ok:5}}).calcExercicios("7","2026-01-20");
