...
/* Quanto a data da prova precisa andar para os topicos dos dias perdidos
   caberem. Espelha a regra de redistribuirTopicos: UM topico extra por dia de
   conteudo livre (posicoes 0 a 4 do ciclo), a partir de hoje, pulando dia ja
   estudado e dia que ja recebeu extra.

   `vagas` conta so as vagas ate a Revisao Geral atual (getLimiteConteudo), que
   e onde o conteudo novo pode entrar. Se elas bastam, `cabe` e true e a prova
   nao precisa mudar. Se nao bastam, a caminhada continua adiante e a nova prova
   e o ultimo dia de conteudo + 8, a mesma regra de calcPrazoConteudo: Revisao
   Geral em +7, prova em +8.

   Devolve null sem prova, sem inicio ou quando nao ha topico a recuperar. */
function calcAdiamentoProva(missedKeys,hojeRef){
  if(!STATE.inicio||!STATE.prova) return null;
  let pendentes=0;
  (missedKeys||[]).forEach(k=>{ pendentes+=getTopicosDiaBase(k).length; });
  if(!pendentes) return null;
  const hoje=hojeRef?parseDate(hojeRef):new Date(); hoje.setHours(0,0,0,0);
  const lim=getLimiteConteudo();
  let vagas=0, restam=pendentes, ultimo=null;
  const d=new Date(hoje);
  for(let guard=0;guard<1500&&restam>0;guard++){
    const k=fmt(d);
    const pos=getCicloPos(k);
    if(pos>=0&&pos<=4&&!(STATE.dias[k]&&STATE.dias[k].percepcao)&&getExtrasDoDia(k).length===0){
      if(lim&&d<lim) vagas++;
      restam--; ultimo=k;
    }
    d.setDate(d.getDate()+1);
  }
  if(restam>0||!ultimo) return null;
  const cabe=vagas>=pendentes;
  const pv=parseDate(ultimo); pv.setDate(pv.getDate()+8);
  const provaNova=cabe?STATE.prova:(fmt(pv)>STATE.prova?fmt(pv):STATE.prova);
  const dias=Math.round((parseDate(provaNova)-parseDate(STATE.prova))/86400000);
  return {topicos:pendentes,cabe,vagas,sobram:Math.max(0,pendentes-vagas),
          provaAtual:STATE.prova,provaNova,dias,ultimoConteudo:ultimo};
}
...
function calcRevisoes(hojeRef){
  ...
  Object.entries(STATE.dias).forEach(([k,v])=>{
    /* LEGADO, mas VIVO: a opção que criava dias `recuperado` saiu do modal em
       05/09/2026, e quem usou antes disso tem esses dias salvos. Não remover.
...
/* Acumulado por DIA das respostas de exercicio: { "AAAA-MM-DD": {n, ok} }.
   O historico por questao (STATE.questoes) guarda so a ULTIMA data de cada uma,
   entao ele nao responde "quantas questoes por dia" nem "quanto acertei nos
   ultimos 7 dias". Esta e a base do Painel de Exercicios do dashboard.
   E um agregado minusculo: uma linha por dia, e nao uma por resposta. */
function _exDias(){
...
/* PAINEL DE EXERCICIOS do dashboard. Compila STATE.exDias, que e alimentado
   pelo menu Exercicios a cada resposta.

   periodo: "hoje" | "7" | "30" | "sempre".
   O anel de desempenho e os contadores respeitam o periodo. A media diaria e a
   variacao sao SEMPRE de 7 dias contra os 7 anteriores, porque comparar uma
   janela movel com ela mesma nao diz nada; o rotulo na tela diz "(7 dias)".
   A serie de barras acompanha o periodo: 8 dias nos recortes curtos, 30 nos
   longos, para a barra nao virar risco de cabelo.

   Devolve sempre um objeto, mesmo sem nenhuma resposta: a tela decide se mostra
   o vazio, o engine nao devolve null para o caso normal de aluno novo. */
function calcExercicios(periodo,hojeRef){
  const dias=(STATE.exDias&&typeof STATE.exDias==="object")?STATE.exDias:{};
  const hoje=hojeRef?parseDate(hojeRef):new Date();
  hoje.setHours(0,0,0,0);
  const p=periodo||"sempre";

  function janela(n){                       // as n datas ate hoje, inclusive
    const out=[];
    for(let i=n-1;i>=0;i--){
      const d=new Date(hoje); d.setDate(d.getDate()-i);
      out.push(fmt(d));
  ...
  function soma(chaves){
    let n=0, ok=0;
    chaves.forEach(function(k){ const r=dias[k]; if(r){ n+=(r.n||0); ok+=(r.ok||0); } });
    return {n:n,ok:ok};
  }

  const chavesPeriodo = p==="hoje" ? [fmt(hoje)]
                      : p==="7"    ? janela(7)
                      : p==="30"   ? janela(30)
                      : Object.keys(dias).sort();
  const tot=soma(chavesPeriodo);
  const erros=tot.n-tot.ok;
  const pct=tot.n?Math.round((tot.ok/tot.n)*100):0;

  const nBarras=(p==="30"||p==="sempre")?30:8;
  const serie=janela(nBarras).map(function(k){
    const r=dias[k]||{n:0,ok:0};
    const d=parseDate(k);
    return {key:k,n:r.n||0,ok:r.ok||0,
            rotulo:d.toLocaleDateString("pt-BR",{day:"2-digit",month:"2-digit"}),
            dow:["Dom","Seg","Ter","Qua","Qui","Sex","Sáb"][d.getDay()],
            hoje:k===fmt(hoje)};
  });

  const s7=soma(janela(7));
  const ant=[];
  for(let i=13;i>=7;i--){ const d=new Date(hoje); d.setDate(d.getDate()-i); ant.push(fmt(d)); }
  const s14=soma(ant);
  const mediaDiaria=Math.round(s7.n/7);
  const variacao=s14.n?Math.round(((s7.n-s14.n)/s14.n)*100):null;

  let totalGeral=0;
  Object.keys(dias).forEach(function(k){ totalGeral+=(dias[k].n||0); });

  return {periodo:p,total:tot.n,acertos:tot.ok,erros:erros,pct:pct,
          serie:serie,mediaDiaria:mediaDiaria,variacao:variacao,
          totalGeral:totalGeral,temDados:totalGeral>0};
}
