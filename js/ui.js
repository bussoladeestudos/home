...
const ACTIONS={
  ...
  limparFilaRecuperacao:()=>limparFilaRecuperacao(),
  ...
  setStarHojeTopico:d=>setStarHojeTopico(d.key,+d.ti,+d.n),
...
function calcProgresso(){
  ...
  for(let d=new Date(ini);d<=fimRef;d.setDate(d.getDate()+1)){
    const dow=d.getDay();
    if(isDiaEstudo(dow)&&!retornoTecnicoSemPendencia(fmt(d))) total++;
  }
...
function calcProgressoDetalhado(){
  ...
  for(let d=new Date(ini);d<=pro;d.setDate(d.getDate()+1)){
    const dow=d.getDay();
    if(isDiaEstudo(dow)&&!retornoTecnicoSemPendencia(fmt(d))) totalProva++;
  }
  const base=Math.max(total,done,1);
...
function reiniciarCompleto(){
  /* Reiniciar tem de apagar TUDO que está preso ao cronograma antigo, e não só
     STATE.dias. Era o bug: a fila de recuperação sobrevivia ao reinício e
     continuava desenhando a lista inteira no mesmo Retorno Técnico, para
     sempre, porque nada no sistema jamais esvaziava essa fila.
     O que NÃO se apaga aqui, de propósito: as anotações da semana e o
     histórico de exercícios (STATE.questoes). São do aluno e não são
     cronograma; quem quiser apagar exercício apaga em outro lugar. */
  _confirmReinicio=false;
  STATE.dias={};
  STATE.extrasPorDia={};
  STATE.recuperacao=[];
  delete STATE.recuperacaoData;
  delete STATE.redistribuindo; delete STATE.diasRedistribuir;
  /* Com o cronograma zerado e a data de início no passado, todos os dias
     anteriores viram "sem registro" na hora. Segurar o aviso pelo resto do dia
     evita o modal de dias perdidos abrir em cima do clique de reiniciar. */
...
/* ── PAINEL DE EXERCÍCIOS ──────────────────────────────────────────
   ...
   QUANTO O ALUNO ACERTA (o anel, com o percentual no centro) e QUANTO ELE
   TREINA (as barras por dia). Uma sem a outra engana: 90% de acerto em cinco
   questões não é domínio, e 200 questões por dia com 40% de acerto é esforço
   no lugar errado.

   O anel é um medidor de uma medida só, e não um gráfico de pizza: o número
   grande no meio é a informação, o anel é a régua. Por isso são dois arcos e
   não seis fatias.

   Verde e vermelho aqui não são cores de série, são estado: certo e errado. É
   a mesma escala das estrelas, do mapa de calor e da cobertura, então o aluno
   já sabe lê-la sem legenda nova. */
let _exPeriodo="7";

function exSetPeriodo(p){ _exPeriodo=p||"7"; renderPainelExercicios(); }

function _anelDesempenho(pct,total){
  /* O arco é SEMPRE verde e a trilha SEMPRE vermelha, porque arco e trilha são
     acertos e erros, e não a nota. Quem julga é o número no meio, que troca de
     cor pela faixa (70% é o corte da CPA). Pintar o arco de vermelho num aluno
     de 40% deixaria a legenda "verde = acertos" mentindo em cima do desenho. */
  const R=52, C=2*Math.PI*R;
  const acerto=total?(pct/100)*C:0;
  const faixa=pct>=70?"bom":pct>=50?"medio":"ruim";
  return `<svg class="pex-anel" viewBox="0 0 130 130" role="img"
      aria-label="${total?pct+"% de acertos em "+total+" questões":"sem questões respondidas"}">
    <circle cx="65" cy="65" r="${R}" fill="none" stroke="${total?"var(--red-viv)":"var(--gray-200)"}" stroke-width="15"/>
    ${total?`<circle cx="65" cy="65" r="${R}" fill="none" stroke="var(--green-viv)" stroke-width="15"
      stroke-dasharray="${acerto.toFixed(1)} ${(C-acerto).toFixed(1)}"
      transform="rotate(-90 65 65)"/>`:""}
    <text x="65" y="62" class="pex-anel-num ${faixa}" text-anchor="middle">${total?pct+"%":"—"}</text>
    <text x="65" y="79" class="pex-anel-lbl" text-anchor="middle">de acertos</text>
  </svg>`;
}

function _barrasPorDia(serie){
  /* Marca fina: a barra tem largura máxima e fica centrada na coluna, em vez de
     esticar até encostar na vizinha. Bloco grosso e saturado lê alto e cansa;
     a informação aqui é a ALTURA, e não a área pintada. */
  const max=Math.max(1,...serie.map(d=>d.n));
  const compacto=serie.length>12;
  return `<div class="pex-barras${compacto?" compacto":""}" role="img"
      aria-label="Questões respondidas por dia nos últimos ${serie.length} dias">
    ${serie.map(d=>{
      const h=Math.round((d.n/max)*100);
      const t=`${d.n} ${d.n===1?"questão":"questões"} em ${d.rotulo}${d.n?` · ${Math.round((d.ok/d.n)*100)}% de acerto`:""}`;
      return `<div class="pex-col" title="${esc(t)}">
        <div class="pex-col-n">${d.n||""}</div>
        <div class="pex-col-trilho"><div class="pex-col-barra${d.hoje?" hoje":""}" style="height:${d.n?Math.max(h,2):0}%"></div></div>
        <div class="pex-col-dia">${compacto?d.rotulo.slice(0,2):d.rotulo}</div>
        ${compacto?"":`<div class="pex-col-dow">${d.hoje?"Hoje":d.dow}</div>`}
      </div>`;
    }).join("")}
  </div>`;
}

function renderPainelExercicios(){
  const el=document.getElementById("painelExercicios");
  if(!el) return;
  if(!STATE.prefeitura){ el.innerHTML=""; return; }
  let r=null; try{ r=calcExercicios(_exPeriodo); }catch(e){}
  if(!r) { el.innerHTML=""; return; }

  if(!r.temDados){
    el.innerHTML=`<div class="painel-sec">
      <div class="painel-sec-head">
        <span class="painel-sec-tit">🎯 Painel de Exercícios</span>
        <span class="painel-sec-sub">acompanhe seu desempenho e a evolução ao longo do tempo</span>
      </div>
      <div class="pex-vazio">
        <div class="pex-vazio-ic">✏️</div>
        <div class="pex-vazio-tit">Você ainda não respondeu nenhuma questão</div>
        <div class="pex-vazio-txt">Resolva exercícios e este painel passa a mostrar seu percentual de acertos, o ritmo diário e a evolução por semana.</div>
        <button type="button" class="pex-vazio-btn" data-action="navTo" data-page="exercicios">Ir para os Exercícios →</button>
      </div>
    </div>`;
    return;
  }

  const chip=(v,t)=>`<button type="button" class="pex-chip${_exPeriodo===v?" on":""}" data-action="exSetPeriodo" data-p="${v}">${t}</button>`;
  const varTxt=r.variacao===null
    ? `<span class="pex-var neutro">sem semana anterior para comparar</span>`
    : `<span class="pex-var ${r.variacao>=0?"sobe":"desce"}">${r.variacao>=0?"↑":"↓"} ${Math.abs(r.variacao)}% em relação à semana anterior</span>`;
  const rotuloPeriodo={hoje:"hoje",7:"nos últimos 7 dias",30:"nos últimos 30 dias",sempre:"desde o início"}[_exPeriodo];
    ...
    <div class="pex-grid">
      <div class="pex-card">
        <div class="pex-card-head">
          <span class="pex-card-tit">Desempenho geral</span>
          <span class="pex-card-sub">percentual de acertos ${rotuloPeriodo}</span>
        </div>
        <div class="pex-anel-wrap">
          <div class="pex-leg">
            <div class="pex-leg-top"><span class="pex-dot verde"></span><span class="pex-leg-pct">${r.pct}%</span></div>
            <div class="pex-leg-t">Acertos</div>
            <div class="pex-leg-q">${r.acertos} ${r.acertos===1?"questão":"questões"}</div>
          </div>
          ${_anelDesempenho(r.pct,r.total)}
          <div class="pex-leg dir">
            <div class="pex-leg-top"><span class="pex-dot vermelho"></span><span class="pex-leg-pct erro">${100-r.pct}%</span></div>
            <div class="pex-leg-t">Erros</div>
            <div class="pex-leg-q">${r.erros} ${r.erros===1?"questão":"questões"}</div>
          </div>
        </div>
        <div class="pex-chips">${chip("hoje","Hoje")}${chip("7","7 dias")}${chip("30","30 dias")}${chip("sempre","Sempre")}</div>
      </div>
      <div class="pex-card">
        <div class="pex-card-head">
          <span class="pex-card-tit">Questões respondidas por dia</span>
          <span class="pex-card-sub">últimos ${r.serie.length} dias</span>
        </div>
        ${_barrasPorDia(r.serie)}
        <div class="pex-rodape">
          <span class="pex-media"><strong>${r.mediaDiaria}</strong> ${r.mediaDiaria===1?"questão":"questões"} por dia, média dos últimos 7 dias</span>
          ${varTxt}
        </div>
      </div>
    </div>
...
function renderHabito(){
  /* Os três cards do hábito vivem em contêineres separados desde 05/09/2026:
     "Quando você estuda" fica ao lado da Aderência, e Consistência e Desafios
     descem para a segunda linha do Painel de Consistência. */
  const elR=document.getElementById("habRitmo");
  const elM=document.getElementById("habMes");
  const elD=document.getElementById("habDesafios");
  if(!elR||!elM||!elD) return;
  if(!STATE.inicio){ elR.innerHTML=elM.innerHTML=elD.innerHTML=""; return; }
  ...
  const r=hm.resumo;
  elR.innerHTML=`<div class="hab-card">
  ...
  elM.innerHTML=`<div class="hab-card">
  ...
  elD.innerHTML=`<div class="hab-card gam">
...
function getFocoHojeLabel(){
  ...
  const topicos=getTopicosDiaBase(hk);
  return topicos.length?topicos.map(t=>`${t.mat}: ${t.top}`).join(" · "):"Revisão Geral";
}
...
function buildStatusCard(o){
  ...
  return `<div${idAttr} style="background:${cardBg};border:1px solid ${cardBorder};border-radius:16px;margin-bottom:.8rem;overflow:hidden;box-shadow:0 1px 3px rgba(60,45,20,.04)">
    ...
    <div id="sc-body-${o.id||""}" style="${bodyOpenStyle}">
      <div style="padding:0 1.15rem .6rem">${bullets}${o.bodyHtml||""}</div>
...
function renderMaterias(){
  ...
  const itensHtml=materias.map((m,i)=>{
    ...
    /* Resumo que substitui a lista quando a matéria está fechada: o aluno
       precisa saber o tamanho da matéria antes de decidir abrir. */
    const nAval=tops.filter(x=>{
      const inf=agenda[_normTexto(m.nome)+"|"+_normTexto(x)];
      return inf&&inf.percepcao;
    }).length;
    const resumo=`${tops.length} tópico${tops.length===1?"":"s"}`+(nAval?` · ${nAval} já avaliado${nAval===1?"":"s"}`:"");
    return`<div class="mat-item" data-busca="${esc(m.nome.toLowerCase())}" data-i="${i}">
  ...
  const buscaHtml=`
    ...
    <div class="mat-busca-ajuda">
      <span>${totalComTexto} de ${totalTop} tópicos já têm texto publicado. Toque na matéria para abrir a lista e no tópico com 📖 para ler.</span>
      <button type="button" class="mat-todas" data-action="toggleTodasMaterias" id="matTodasBtn">Abrir todas</button>
    </div>
...
let _buscaConteudo="";
/* Matérias abertas no acordeão. Fechadas por padrão: a tela abre limpa e o
   aluno escolhe onde entrar. Preferência só da sessão, não vai para o STATE. */
let _matAbertas=new Set();
...
function toggleTodasMaterias(){
  ...
  const todasAbertas=itens.length>0&&itens.every(it=>_matAbertas.has(String(it.dataset.i)));
  _matAbertas=todasAbertas?new Set():new Set(itens.map(it=>String(it.dataset.i)));
  aplicarBuscaConteudo();
}
...
function aplicarBuscaConteudo(){
  ...
  itens.forEach(it=>{
    ...
    it.hidden=!mostra;
    if(mostra) visiveis++;
    /* Buscando, a matéria com resultado abre sozinha: senão o aluno digita
       e não vê nada. Sem busca, vale o que ele abriu com o clique. */
    const aberta=termo?(matCasa||algum):_matAbertas.has(String(it.dataset.i));
    if(aberta) abertas++;
    it.classList.toggle("aberta",aberta);
    const subs=it.querySelector(".mat-subs");
    if(subs) subs.hidden=!aberta;
    const cab=it.querySelector(".mat-item-header");
    if(cab) cab.setAttribute("aria-expanded",aberta?"true":"false");
  });
  const btnTodas=document.getElementById("matTodasBtn");
  if(btnTodas){
    btnTodas.hidden=!!termo;
    btnTodas.textContent=(itens.length&&abertas>=itens.length)?"Fechar todas":"Abrir todas";
  }
...
function abrirSimulado(key){
  ...
  delete STATE.dias[key].simuladoResultadoId;
  save(); renderSemana(); renderSimuladoPage(); updateNavBadges();
}
...
function redistribuirTopicos(missedKeys){
  ...
  const hoje=new Date(); hoje.setHours(0,0,0,0);
  /* O conteudo novo para na Revisao Geral, nao na vespera da prova: a ultima
     semana e de treino. Antes o limite era a propria data da prova e topico
     redistribuido caia dentro da reta final. */
  const limite=getLimiteConteudo();
...
function _exMarcado(mat,top){
  return _exFiltro.topicos.some(t=>t.mat===mat&&t.top===top);
}
...
function renderExercicios(){
  ...
  /* Lista de tópicos marcáveis. Com 43 tópicos só em Produtos, e 108 no edital
     inteiro, um <select> de escolha única não serve mais: o aluno não acha o
  ...
  const listaHtml=grupos.map(m=>{
    ...
    const linhas=tops.map(t=>{
      const i=_exLista.length;
      const n=getQuestoes(STATE.prefeitura,m.nome,t).length;
      _exLista.push({mat:m.nome,top:t,n});
      const on=_exMarcado(m.nome,t);
      return `<button type="button" class="ex-top-item${on?" on":""}" data-action="exToggleTopico" data-i="${i}"
        data-mat="${esc(m.nome)}" data-busca="${esc(m.nome+" "+t)}" aria-pressed="${on?"true":"false"}">
        <span class="ex-top-check">${on?"✓":""}</span>
        <span class="ex-top-nome">${esc(t)}</span>
        <span class="ex-top-n">${n}</span>
      </button>`;
    }).join("");
    const cab=grupos.length>1
      ? `<div class="ex-top-grupo" data-grupo="${esc(m.nome)}">${esc(m.nome)}</div>` : "";
    return cab+linhas;
  }).join("");
        ...
        <div class="ex-tops">
          <div class="ex-tops-topo">
            <input type="text" class="form-input ex-tops-busca" id="exBuscaTop" data-input="exBuscarTopico"
                   placeholder="Buscar tópico pelo nome" value="${esc(_exBuscaTop)}" autocomplete="off" spellcheck="false">
            <button type="button" class="ex-btn-mini" data-action="exMarcarVisiveis">Marcar os visíveis</button>
            <button type="button" class="ex-btn-mini" data-action="exLimparTopicos">Limpar</button>
          </div>
          <div class="ex-tops-lista" id="exTopsLista">${listaHtml}</div>
          <div class="ex-tops-nada" id="exTopsNada" hidden>Nenhum tópico com esse nome.</div>
          <div class="ex-tops-sel" id="exTopsSel"></div>
        </div>
      </div>
      <div class="ex-linha">
  ...
  /* Depois de redesenhar, reaplica o texto da busca sobre as linhas novas e
     ...
     visual da busca que o aluno tinha acabado de digitar. */
  exBuscarTopico(_exBuscaTop);
}
...
/* Marca e desmarca SEM redesenhar o painel. Redesenhar apagaria o foco do
   campo de busca, e o aluno marca vários tópicos seguidos digitando. */
function exToggleTopico(i){
  const item=_exLista[+i]; if(!item) return;
  const k=_exFiltro.topicos.findIndex(t=>t.mat===item.mat&&t.top===item.top);
  const ligar=(k===-1);
  if(ligar) _exFiltro.topicos.push({mat:item.mat,top:item.top});
  else _exFiltro.topicos.splice(k,1);
  const el=document.querySelector('#exTopsLista .ex-top-item[data-i="'+i+'"]');
  if(el){
    el.classList.toggle("on",ligar);
    el.setAttribute("aria-pressed",ligar?"true":"false");
    const c=el.querySelector(".ex-top-check"); if(c) c.textContent=ligar?"✓":"";
  }
  _exAtualizarContagem();
}

/* Busca por nome. Esconde e mostra o que já está desenhado, em vez de
   redesenhar: é o que preserva o cursor. Mesma solução da busca do
   Conteúdo programático. */
function exBuscarTopico(v){
  _exBuscaTop=v||"";
  const alvo=_normBusca(_exBuscaTop);
  const linhas=document.querySelectorAll("#exTopsLista .ex-top-item");
  let visiveis=0;
  linhas.forEach(el=>{
    const bate=!alvo||_normBusca(el.dataset.busca||"").indexOf(alvo)>-1;
    el.hidden=!bate;
    if(bate) visiveis++;
  });
  document.querySelectorAll("#exTopsLista .ex-top-grupo").forEach(h=>{
    const m=h.dataset.grupo||"";
    const algum=[...document.querySelectorAll("#exTopsLista .ex-top-item")]
      .some(e=>!e.hidden&&(e.dataset.mat||"")===m);
    h.hidden=!algum;
  });
  const nada=document.getElementById("exTopsNada");
  if(nada) nada.hidden=(visiveis>0);
  _exAtualizarContagem();
}

function exMarcarVisiveis(){
  document.querySelectorAll("#exTopsLista .ex-top-item").forEach(el=>{
    if(el.hidden) return;
    const item=_exLista[+el.dataset.i]; if(!item) return;
    if(_exMarcado(item.mat,item.top)) return;
    _exFiltro.topicos.push({mat:item.mat,top:item.top});
    el.classList.add("on"); el.setAttribute("aria-pressed","true");
    const c=el.querySelector(".ex-top-check"); if(c) c.textContent="✓";
  });
  _exAtualizarContagem();
}

function exLimparTopicos(){
  _exFiltro.topicos=[];
  document.querySelectorAll("#exTopsLista .ex-top-item").forEach(el=>{
...
/* Atualiza só os números do rodapé. Chamado por toda marcação e por toda
   digitação na busca, então tem de ser barato e não redesenhar nada. */
function _exAtualizarContagem(){
  ...
  if(marc){
    ...
    if(!n){
      ...
      const nomes=_exFiltro.topicos.slice(0,4).map(t=>esc(t.top)).join(" · ");
      const resto=n>4?` e mais ${n-4}`:"";
      marc.innerHTML=`<strong>${n} ${n===1?"tópico":"tópicos"}</strong>: ${nomes}${resto}`;
    }
    marc.classList.toggle("on",n>0);
  }
}
...
function exSair(){
  ...
  if(revisao&&!_exSessao.resultadoSalvo&&!confirm("Sair desta sessão? As respostas já dadas ficam no histórico, mas a nota final só é salva ao responder todas as questões.")) return;
  _exSessao=null;
  if(revisao) navTo(revisao.tipo==="mini"?"simulado":"revisoes"); else renderExercicios();
  window.scrollTo(0,0);
}
...
function renderExercicioRun(){
  ...
  el.innerHTML=`
    <div class="ex-run">
      ${S.revisao?`<h3 class="rev-sessao-titulo">${S.revisao.tipo==="mini"?"Mini Simulado":"Revisão"} ${esc(S.revisao.num)}</h3>`:""}
...
/* Aproveitamento por tópico, do pior para o melhor. O percentual é o que
   importa aqui: 3 de 5 e 30 de 50 são o mesmo desempenho, e é essa medida que
   um dia vai alimentar o nível de confiança. A fração fica ao lado porque
   3 de 5 e 30 de 50 têm confiança estatística muito diferente. */
function _exResumoPorTopico(porTopico){
  const chaves=Object.keys(porTopico);
  /* Se tudo veio da mesma matéria, o nome dela repetido em toda linha só rouba
     espaço do que importa, que é o nome do tópico. */
  const umaMateria=new Set(chaves.map(k=>porTopico[k].mat)).size===1;
  const linhas=chaves.map(k=>{
    const r=porTopico[k];
    return {nome:umaMateria?r.top:k,ok:r.ok,n:r.n,pct:Math.round((r.ok/r.n)*100)};
  }).sort((a,b)=>a.pct-b.pct||b.n-a.n);
  if(!linhas.length) return "";
  const abaixo=linhas.filter(l=>l.pct<70).length;
  const recado=abaixo
    ? `${abaixo} ${abaixo===1?"tópico ficou":"tópicos ficaram"} abaixo dos 70%. ${abaixo===1?"Ele aparece":"Eles aparecem"} primeiro na lista.`
    : "Todos os tópicos acima dos 70%.";
  return `<div class="ex-fim-titulo">Aproveitamento por tópico</div>
    <div class="ex-fim-lista">${linhas.map(l=>{
      const c=l.pct>=70?"cb-green":l.pct>=50?"cb-yellow":"cb-red";
      const cor=l.pct>=70?"var(--green)":l.pct>=50?"var(--yellow-viv)":"var(--red-viv)";
      return `<div class="ex-fim-item">
        <span class="ex-fim-nome">${esc(l.nome)}</span>
        <span class="ex-fim-frac">${l.ok} de ${l.n}</span>
        <span class="ex-fim-badge ${c}">${l.pct}%</span>
        <span class="ex-fim-bar"><i style="width:${l.pct}%;background:${cor}"></i></span>
      </div>`;
    }).join("")}</div>
...
function renderExercicioFim(){
  ...
  S.itens.forEach(it=>{
    ...
    if(!porTopico[k]) porTopico[k]={ok:0,n:0,mat:it.mat,top:it.top};
...
function renderDiaNormal(dia,idx,key,est,isHoje,isPast,nomeDia){
  ...
  /* LEGADO, mas VIVO. A opção "Mandar para o Retorno Técnico" saiu do modal em
     05/09/2026, então nenhum dia novo nasce com `recuperado`. Quem usou a opção
     antes disso tem dias assim salvos no navegador e na nuvem, e eles precisam
     continuar desenhando certo. Não apague este ramo, nem o tratamento de
     `recuperado` em engine.calcRevisoes, getMissedDays e isSemanaCompleta.
...
/* Tira da fila de recuperação o item que acabou de ser avaliado. Sem isto a
   fila só crescia: nada, em lugar nenhum, jamais removia um item dela. */
/* Saída de emergência. Se a lista ficar presa por qualquer motivo, o aluno
   limpa daqui em vez de ficar olhando para ela todo Retorno Técnico. Não
   perde conteúdo: os dias de origem continuam marcados e o motor segue
   puxando esses tópicos pelas revisões de 7 e 30 dias. */
function limparFilaRecuperacao(){
  STATE.recuperacao=[];
  delete STATE.recuperacaoData;
  save(); renderTudo();
  showToast("Lista de recuperação limpa.");
}

function _tirarDaFilaRecuperacao(key,ti){
  if(!Array.isArray(STATE.recuperacao)||!STATE.recuperacao.length) return;
  const temTi=(ti!=null&&ti!=="");
  STATE.recuperacao=STATE.recuperacao.filter(it=>{
    if(!it||typeof it!=="object"||it.key!==key) return true;
    if(!temTi) return false;                       // dia de tópico único: sai tudo daquele dia
    return String(it.topIdx)!==String(ti);
  });
  if(!STATE.recuperacao.length) delete STATE.recuperacaoData;
}
...
function setSabStar(key,n,source,ti,sabKey){
  ...
  _tirarDaFilaRecuperacao(key,ti);
  save();
  renderTudo();
...
function renderDiaRetornoTecnico(dia,key,est,isHoje,isPast,fraquezas){
  ...
  let itens="";
  if(temRecuperacao){
    itens=`<div class="sab-rec-cab">
      <span style="font-size:.72rem;font-weight:700;color:var(--red)">🔴 Recuperação de dias perdidos:</span>
      <button type="button" class="sab-rec-limpar" data-action="limparFilaRecuperacao" title="Tira estes tópicos da lista. O conteúdo continua voltando pelas revisões.">Limpar</button>
    </div>`;
  ...
  // e por isso a recuperação apagava a lista de reavaliação da semana.
  {
    ...
    if(itensPendentes.length>0){
      ...
      if(!temRecuperacao){
        ...
        if(algumEstudado){
          itens=`<div class="sabado-empty">🏆 Todos os tópicos desta semana com Alta Confiança! Aproveite para revisar o conteúdo e resolver exercícios.</div>`;
        } else {
...
let _missedDays = [];
let _adiamento = null;   // medida de calcAdiamentoProva, calculada ao abrir o modal
...
const REC_OPCOES=["redistribuir","adiar","pular"];
...
function checkRecovery(){
  ...
  try{ _adiamento=calcAdiamentoProva(missed); }catch(e){ _adiamento=null; }
  ...
  renderOpcoesRecuperacao();
  // Reset visual das opções
  REC_OPCOES.forEach(o=>{
...
/* Escreve nas opções o que de fato vai acontecer, com número e data.
   O aluno decide melhor vendo o preço de cada saída antes de clicar. */
function renderOpcoesRecuperacao(){
  const a=_adiamento;
  const dTit=document.getElementById("recTit-redistribuir");
  const dDesc=document.getElementById("recDesc-redistribuir");
  const optAdiar=document.getElementById("recOpt-adiar");
  const aTit=document.getElementById("recTit-adiar");
  const aDesc=document.getElementById("recDesc-adiar");
  if(!dDesc||!optAdiar) return;

  if(a){
    const n=a.topicos, so=a.sobram;
    const ate=_dataBR(a.ultimoConteudo);
    const alvo=n===1?"O tópico perdido entra":`Os ${n} tópicos perdidos entram`;
    ...
    if(a.cabe){
      ...
      dDesc.textContent=`${alvo} um por dia, mas ${so===1?"1 não cabe":so+" não cabem"} antes da `
        ...
        +`A data da prova continua a mesma.`;
    }
  } else {
    dTit.textContent="Intensificar os próximos dias";
    dDesc.textContent="Os tópicos perdidos entram nos próximos dias, um por dia, e a data da prova continua a mesma.";
  }

  // Adiar só faz sentido quando o conteúdo NÃO cabe no prazo atual.
  // Cabendo, a opção seria um clique que não muda nada.
  if(a&&!a.cabe){
    optAdiar.hidden=false;
    aTit.textContent=`Adiar a prova para ${_dataBR(a.provaNova)}`;
    aDesc.textContent=`A prova anda ${a.dias} dia${a.dias>1?"s":""} e `
      +`${a.topicos===1?"o tópico perdido cabe":"os "+a.topicos+" tópicos perdidos cabem"} `
      +`sem apertar o ritmo. Escolha esta opção se ainda não agendou o exame.`;
  } else {
    optAdiar.hidden=true;
    if(_recoveryOption==="adiar") _recoveryOption=null;
  }
}
...
function selectRecovery(option){
  ...
  btn.disabled=false;
  const labels={
    "redistribuir":"\u26A1 Intensificar os pr\u00F3ximos dias",
    "adiar":_adiamento?`\uD83D\uDCC5 Adiar a prova para ${_dataBR(_adiamento.provaNova)}`:"\uD83D\uDCC5 Adiar a prova",
    "pular":"\u27A1\uFE0F Pular e seguir em frente"};
...
function applyRecovery(){
  ...
  } else if(_recoveryOption==="adiar"){
    /* ADIAR A PROVA. O aluno não aperta o ritmo: a data alvo anda o suficiente
       para o conteúdo perdido caber e só então a redistribuição roda.
       A ORDEM IMPORTA: redistribuirTopicos lê getLimiteConteudo(), que é
       derivado de STATE.prova. Redistribuir antes de mover a data faria o
       conteúdo esbarrar no limite antigo. */
    if(_adiamento&&!_adiamento.cabe) STATE.prova=_adiamento.provaNova;
    const n=redistribuirTopicos(_missedDays);
    if(n>0){ STATE.redistribuindo=true; STATE.diasRedistribuir=_missedDays.length; }
    _missedDays.forEach(k=>{ if(!STATE.dias[k]) STATE.dias[k]={}; STATE.dias[k].transferido=true; });
  ...
  renderTudo();
  navTo("hoje");
  showToast(_recoveryOption==="adiar"&&_adiamento&&!_adiamento.cabe
    ? `\u2705 Prova adiada para ${_dataBR(_adiamento.provaNova)} e conte\u00FAdo redistribu\u00EDdo.`
...
function revTopicos(bloco){
  ...
  return [...pares.values()].map(t=>{
    const vistos=new Set();
    const itens=listarQuestoes(STATE.prefeitura,{materia:t.mat,topicos:[t]}).filter(it=>{
      if(vistos.has(it.q.id)) return false; vistos.add(it.q.id); return true;
    });
    return {mat:t.mat,top:t.top,itens};
  });
}
function revResultadoHtml(r){
  if(!r) return "";
  return `<div class="rev-resultado"><strong>Último resultado: ${esc(r.nota)} / 10 · ${esc(r.pct)}%</strong>
...
function simConfiguracaoHtml(bloco,cfg){
  ...
  if(bloco.isFutura){
    ...
    return `${resultado}<section class="rev-questoes" aria-label="${esc(cfg.titulo)}">
      ...
      <p>Disponível na data planejada. ${tops.length>SIM_MAX_LISTA?"Matérias previstas:":"Tópicos previstos:"}</p>
...
function rgBloco(key){
  ...
  return {tipo:"geral",num:key,key,topicos:rgTopicosEdital(),isFutura:key>fmt(new Date())};
}
...
function renderTopicosHoje(key,topicos){
  ...
  const est=STATE.dias[key]||{},multi=topicos.length>1;
  return topicos.map((t,i)=>{
    const n=multi?((est.estrelasList||{})[i]||0):(est.estrelas||nivelToStars(est.percepcao||""));
    return `<section class="hoje-topico" aria-label="${esc(t.mat+": "+t.top)}">
      ${multi?`<div class="hoje-topico-materia">${esc(i+1)}. ${esc(t.mat)}</div>`:""}
      <h3>${esc(t.top)}</h3>
      ${_btnConteudo(t.mat,t.top)}${_btnAula(t.mat,t.top)}
      <p class="hoje-topico-avaliar">${n?"Sua confiança neste tópico:":"Concluiu este tópico? Avalie sua confiança:"}</p>
      <div class="hoje-topico-estrelas">${[1,2,3,4,5].map(v=>`<button type="button" ${multi?'data-action="setStarHojeTopico"':'data-action="setStarHoje"'} data-key="${esc(key)}" data-ti="${esc(i)}" data-n="${v}" aria-label="${v} de 5 estrelas para ${esc(t.top)}" aria-pressed="${n===v}" class="${v<=n?"on":""}">★</button>`).join("")}</div>
    </section>`;
  }).join("");
}
function setStarHojeTopico(key,ti,n){
  if(!Number.isInteger(ti)||ti<0||ti>=getTopicosDiaBase(key).length||!Number.isInteger(n)||n<1||n>5) return;
  setStarTopico(key,ti,n);
  renderHoje();
}
